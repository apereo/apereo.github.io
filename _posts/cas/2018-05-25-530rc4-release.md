...
- CAS test coverage has improved slightly with a number of additional tests where now coverage is at `65%` and growing.
...
Encryption and signing keys use to allow CAS to encode a [JWT as a service ticket](https://apereo.github.io/cas/development/installation/Configure-ServiceTicket-JWT.html) may now be defined on a per-service basis.
