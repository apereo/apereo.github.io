...
- SAML2 `InResponseTo` field is now correctly set in response. [Community Contribution]
- JDBC data sources are massaged to correctly handle the minimum number of idle connections. [Community Contribution]
- YAML properties are now correctly loaded both from the embedded web application as well as externalized sources. [Community Contribution]
...
- Regressions that made service properties in the CAS management webapp dysfunctional are now removed/fixed. [Community Contribution]
- LDAP authentication removes the need for separate SASL authentication type option, folding it neatly into `AUTHENTICATED`. [Community Contribution]
