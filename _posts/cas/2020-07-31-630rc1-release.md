...
[Apache JMeter performance tests](https://apereo.github.io/cas/development/high_availability/Performance-Testing-JMeter.html) that ship with CAS are now added to [GitHub Actions](https://github.com/apereo/cas/actions). At this point, only the *CAS* variant is tested and other test categories for SAML2 and OAuth will be gradually added once a CAS runtime context (i.e. WAR Overlay) can be dynamically constructed on-demand with a module selection menu. The goal is to ensure the JMeter test artifacts and scripts are maintainable and manageable from one CAS release to the next. 
