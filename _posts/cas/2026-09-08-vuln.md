...
This is sn [Apereo CAS project vulnerability disclosure](https://apereo.github.io/cas/developer/Sec-Vuln-Response.html), which describes a security vulnerability that mainly affects how the CAS server accepts and processes UI. 
...
This vulnerability allows an unauthenticated attacker to execute arbitrary commands on a CAS server by sending a crafted `theme` header to the public login endpoint. The attack requires no credentials, secrets, user interaction, optional CAS modules, writable files, or outbound connection. This special header value reaches Thymeleaf as part of a view name and can invoke Thymeleaf's Webflow expression parser. This turns a template expression into unrestricted Spring expression evaluation and ultimately command execution with the privileges of the CAS service account.

# Timeline & Credits

The issues were originally reported to the CAS project on September 4th, 2026 and fixed on September 8th, 2026. The issues (almost entirely driven by AI analysis) were reported to the CAS project by third-party researchers (who decided to remain anonymous) and were then further validated and tested by the CAS security team.

Thank you everyone!

# Patching

Patch releases were published on September 8th, 2026. Upgrades to the next patch version for each release should be a drop-in replacement.
...
Modify your CAS overlay to point to version `7.3.8.3`.
