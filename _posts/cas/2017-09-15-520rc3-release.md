...
- [Attribute Consent via LDAP](#attribute-consent-via-ldap)
...
- [Attribute Consent](#attribute-consent)
    ...
    - [MongoDb Consent Storage](#mongodb-consent-storage)
...
- [MongoDb Service Registry](#mongodb-service-registry)
...
In refactoring a number of configuration classes that deal with MongoDb settings, the [MongoDb service registry](https://apereo.github.io/cas/development/installation/Mongo-Service-Management.html) is changed slightly to use a `collection` property rather than `collectionName` in order to keep things consistent across all settings.
...
The registry has long carried the behavior enforce blocking synchronous ticket CRUD operations. In this release candidate, the registry switches to a more asynchronous model to gain maximum performance.
