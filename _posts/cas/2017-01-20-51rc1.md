...
The strategy is more or less the same where `@Configuration` classes still register CAS components as `@Bean`
components. The difference is that each CAS module provides a `spring.factories` file in which configuration
...
Note that while this isn't generally that the average adopter needs to worry about,
this is nonetheless a breaking change **IIF** you have existing classes that are auto-scanned under `org.apereo.cas`.
