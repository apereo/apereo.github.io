...
For some time, CAS has had support for a [REST-like API allowing one to add service definitions](https://apereo.github.io/cas/development/protocol/REST-Protocol.html) into the registry. This API was quite limited in the way it accepted input in terms of different properties of a given service definition or different types of services for various protocols. In this release candidate, this API is improved slightly to accept all types of service definitions in the API request body and making it slightly easier to execute such requests with property authentication and authorization enforcements. 
