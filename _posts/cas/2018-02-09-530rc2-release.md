...
23. Thanksto [@plajko](https://github.com/plajko), a number of MongoDb performance and configuration improvements are included in this release candidate. Additionally, the default size of the embedded Apache Tomcat's thread pool is now increased.
...
OAuth2 user profiles produced by CAS while running as a OAuth identity provider are now routed to the audit log. 
...
<div class="alert alert-info">
  <strong>Beware</strong><br/>As part of this change, the <code>thottler</code> setting that REST and OAuth module settings carried is now removed in favor of the default throttler functionality.</div>
